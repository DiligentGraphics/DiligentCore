...
CComPtr<IDSRDevice> CreateDSRDeviceD3D12(IRenderDevice* pDevice)
{
    ...
    auto pfnD3D12GetInterface       = reinterpret_cast<D3D12GetInterfaceProcType>(GetProcAddress(hD3D12, "D3D12GetInterface"));
...
static DSR_SUPERRES_CREATE_ENGINE_FLAGS SuperResolutionFlagsToDSRFlags(SUPER_RESOLUTION_FLAGS Flags)
{
...
class SuperResolutionD3D12 final : public SuperResolutionBase
{
public:
    SuperResolutionD3D12(IReferenceCounters*        pRefCounters,
...
SuperResolutionD3D12::SuperResolutionD3D12(IReferenceCounters*        pRefCounters,
                                           RenderDeviceD3D12Impl*     pDevice,
                                           const SuperResolutionDesc& Desc,
                                           IDSRDevice*                pDSRDevice) :
    ...
    m_DSRUpscalers(pDevice->GetCommandQueueCount())
{
    ...
    DEV_CHECK_ERR((Desc.Flags & SUPER_RESOLUTION_FLAG_AUTO_EXPOSURE) != 0 || Desc.ExposureFormat != TEX_FORMAT_UNKNOWN,
...
SuperResolutionD3D12::~SuperResolutionD3D12() = default;

void DILIGENT_CALL_TYPE SuperResolutionD3D12::Execute(const ExecuteSuperResolutionAttribs& Attribs)
{
    DEV_CHECK_ERR(Attribs.pContext != nullptr, "Device context must not be null");
    ...
    DEV_CHECK_ERR((m_Desc.Flags & SUPER_RESOLUTION_FLAG_AUTO_EXPOSURE) != 0 || Attribs.pExposureTextureSRV != nullptr,
    ...
    auto* pCtx = ClassPtrCast<DeviceContextD3D12Impl>(Attribs.pContext);
    ...
    // Validate color texture
    {
        ...
        const auto& ViewDesc = Attribs.pColorTextureSRV->GetDesc();
        DEV_CHECK_ERR(ViewDesc.ViewType == TEXTURE_VIEW_SHADER_RESOURCE,
        ...
        DEV_CHECK_ERR(ViewDesc.Format == m_Desc.ColorFormat,
                      "Color texture view '", TexDesc.Name, "' format (", GetTextureFormatAttribs(ViewDesc.Format).Name,
                      ") does not match the expected ColorFormat (", GetTextureFormatAttribs(m_Desc.ColorFormat).Name, ")");
    }

    // Validate output texture
    {
        const auto& TexDesc  = Attribs.pOutputTextureView->GetTexture()->GetDesc();
        const auto& ViewDesc = Attribs.pOutputTextureView->GetDesc();
        DEV_CHECK_ERR(ViewDesc.ViewType == TEXTURE_VIEW_UNORDERED_ACCESS,
        ...
        DEV_CHECK_ERR(ViewDesc.Format == m_Desc.OutputFormat,
                      "Output texture view '", TexDesc.Name, "' format (", GetTextureFormatAttribs(ViewDesc.Format).Name,
                      ") does not match the expected OutputFormat (", GetTextureFormatAttribs(m_Desc.OutputFormat).Name, ")");
    }
    ...
    if (Attribs.pDepthTextureSRV != nullptr)
    {
        ...
        const auto& ViewDesc = Attribs.pDepthTextureSRV->GetDesc();
        DEV_CHECK_ERR(ViewDesc.ViewType == TEXTURE_VIEW_SHADER_RESOURCE,
        ...
        DEV_CHECK_ERR(TexDesc.Width >= m_Desc.InputWidth && TexDesc.Height >= m_Desc.InputHeight,
                      ...
                      ") must be at least the upscaler input resolution (", m_Desc.InputWidth, "x", m_Desc.InputHeight, ")");
        DEV_CHECK_ERR(ViewDesc.Format == m_Desc.DepthFormat,
                      "Depth texture view '", TexDesc.Name, "' format (", GetTextureFormatAttribs(ViewDesc.Format).Name,
                      ") does not match the expected DepthFormat (", GetTextureFormatAttribs(m_Desc.DepthFormat).Name, ")");
    }
    ...
    if (Attribs.pMotionVectorsSRV != nullptr)
    {
        ...
        const auto& ViewDesc = Attribs.pMotionVectorsSRV->GetDesc();
        DEV_CHECK_ERR(ViewDesc.ViewType == TEXTURE_VIEW_SHADER_RESOURCE,
        ...
        DEV_CHECK_ERR(TexDesc.Width >= m_Desc.InputWidth && TexDesc.Height >= m_Desc.InputHeight,
                      ...
                      ") must be at least the upscaler input resolution (", m_Desc.InputWidth, "x", m_Desc.InputHeight, ")");
        DEV_CHECK_ERR(ViewDesc.Format == TEX_FORMAT_RG16_FLOAT,
                      "Motion vectors view '", TexDesc.Name, "' format (", GetTextureFormatAttribs(ViewDesc.Format).Name,
                      ") must be TEX_FORMAT_RG16_FLOAT");
    }

    // Validate exposure texture
    if (Attribs.pExposureTextureSRV != nullptr)
    {
        const auto& TexDesc  = Attribs.pExposureTextureSRV->GetTexture()->GetDesc();
        const auto& ViewDesc = Attribs.pExposureTextureSRV->GetDesc();
        DEV_CHECK_ERR(ViewDesc.ViewType == TEXTURE_VIEW_SHADER_RESOURCE,
                      "Exposure texture view '", TexDesc.Name, "' must be TEXTURE_VIEW_SHADER_RESOURCE");
        DEV_CHECK_ERR(TexDesc.Width == 1 && TexDesc.Height == 1,
                      "Exposure texture '", TexDesc.Name, "' dimensions (", TexDesc.Width, "x", TexDesc.Height,
                      ") must be 1x1");
        DEV_CHECK_ERR(ViewDesc.Format == m_Desc.ExposureFormat,
                      "Exposure texture view '", TexDesc.Name, "' format (", GetTextureFormatAttribs(ViewDesc.Format).Name,
                      ") does not match the expected ExposureFormat (", GetTextureFormatAttribs(m_Desc.ExposureFormat).Name, ")");
    }

    // Validate reactive mask texture
    if (Attribs.pReactiveMaskTextureSRV != nullptr)
    {
        const auto& TexDesc  = Attribs.pReactiveMaskTextureSRV->GetTexture()->GetDesc();
        const auto& ViewDesc = Attribs.pReactiveMaskTextureSRV->GetDesc();
        DEV_CHECK_ERR(ViewDesc.ViewType == TEXTURE_VIEW_SHADER_RESOURCE,
                      "Reactive mask view '", TexDesc.Name, "' must be TEXTURE_VIEW_SHADER_RESOURCE");
        DEV_CHECK_ERR(m_Desc.ReactiveMaskFormat != TEX_FORMAT_UNKNOWN,
        ...
        DEV_CHECK_ERR(TexDesc.Width >= m_Desc.InputWidth && TexDesc.Height >= m_Desc.InputHeight,
                      ...
                      ") must be at least the upscaler input resolution (", m_Desc.InputWidth, "x", m_Desc.InputHeight, ")");
        DEV_CHECK_ERR(ViewDesc.Format == TEX_FORMAT_R8_UNORM,
                      "Reactive mask view '", TexDesc.Name, "' format (", GetTextureFormatAttribs(ViewDesc.Format).Name,
                      ") must be TEX_FORMAT_R8_UNORM");
    }

    // Validate ignore history mask texture
    if (Attribs.pIgnoreHistoryMaskTextureSRV != nullptr)
    {
        const auto& TexDesc  = Attribs.pIgnoreHistoryMaskTextureSRV->GetTexture()->GetDesc();
        const auto& ViewDesc = Attribs.pIgnoreHistoryMaskTextureSRV->GetDesc();
        DEV_CHECK_ERR(ViewDesc.ViewType == TEXTURE_VIEW_SHADER_RESOURCE,
        ...
        DEV_CHECK_ERR(ViewDesc.Format == TEX_FORMAT_R8_UINT,
                      "Ignore history mask view '", TexDesc.Name, "' format (", GetTextureFormatAttribs(ViewDesc.Format).Name,
                      ") must be TEX_FORMAT_R8_UINT");
    }
...
void EnumerateVariantsD3D12(IDSRDevice* pDSRDevice, std::vector<SuperResolutionInfo>& Variants)
{
    DEV_CHECK_ERR(pDSRDevice != nullptr, "DirectSR device must not be null");
    ...
    const Uint32 DSRNumVariants = pDSRDevice->GetNumSuperResVariants();
    for (Uint32 Idx = 0; Idx < DSRNumVariants; ++Idx)
    {
        DSR_SUPERRES_VARIANT_DESC VariantDesc = {};
        if (FAILED(pDSRDevice->GetSuperResVariantDesc(Idx, &VariantDesc)))
            continue;
                            ...
                            SuperResolutionSourceSettings&              Settings)
{
    ...
    const Uint32 NumVariants  = pDSRDevice->GetNumSuperResVariants();
    Uint32       VariantIndex = UINT32_MAX;
    for (Uint32 Idx = 0; Idx < NumVariants; ++Idx)
    {
        DSR_SUPERRES_VARIANT_DESC VariantDesc = {};
        if (SUCCEEDED(pDSRDevice->GetSuperResVariantDesc(Idx, &VariantDesc)))
        {
    ...
    if (HRESULT hr = pDSRDevice->QuerySuperResSourceSettings(VariantIndex, TargetSize, TexFormatToDXGI_Format(Attribs.OutputFormat), DSROptType, DSRCreateFlags, &SourceSettings); SUCCEEDED(hr))
    {
...
{
    DEV_CHECK_ERR(pDSRDevice != nullptr, "DirectSR device must not be null");
    DEV_CHECK_ERR(pDevice != nullptr, "Render device must not be null");

    auto* pDeviceD3D12 = ClassPtrCast<RenderDeviceD3D12Impl>(pDevice);
    auto* pUpscaler    = NEW_RC_OBJ(GetRawAllocator(), "SuperResolutionD3D12 instance", SuperResolutionD3D12)(pDeviceD3D12, Desc, pDSRDevice);
