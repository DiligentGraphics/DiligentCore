...
SUPER_RESOLUTION_BACKEND SuperResolutionFactoryImpl::FindVariant(const INTERFACE_ID& VariantId) const
{
    ...
    {
        for (const SuperResolutionInfo& Info : m_Variants[BackendIdx])
        {
...
void SuperResolutionFactoryImpl::GetSourceSettings(const SuperResolutionSourceSettingsAttribs& Attribs, SuperResolutionSourceSettings& Settings) const
{
    ...
    const SUPER_RESOLUTION_BACKEND Backend = FindVariant(Attribs.VariantId);
    ...
    switch (Backend)
    {
        ...
        case SUPER_RESOLUTION_BACKEND_SOFTWARE:
...
void SuperResolutionFactoryImpl::CreateSuperResolution(const SuperResolutionDesc& Desc, ISuperResolution** ppUpscaler)
{
    ...
    DEV_CHECK_ERR(*ppUpscaler == nullptr, "*ppUpscaler is not null. Overwriting reference to existing object may cause memory leaks");

    *ppUpscaler = nullptr;

    const SUPER_RESOLUTION_BACKEND Backend = FindVariant(Desc.VariantId);
    if (Backend == SUPER_RESOLUTION_BACKEND_COUNT)
    {
...
API_QUALIFIER void CreateSuperResolutionFactory(IRenderDevice* pDevice, ISuperResolutionFactory** ppFactory)
{
    ...
    *ppFactory = nullptr;
    ...
    try
    {
        SuperResolutionFactoryImpl* pFactory = NEW_RC_OBJ(GetRawAllocator(), "SuperResolutionFactoryImpl instance", SuperResolutionFactoryImpl)(pDevice);
