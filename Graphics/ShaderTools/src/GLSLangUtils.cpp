...
private:
    IncludeResult* ReadIncludeFile(const char* IncludePath, CREATE_SHADER_SOURCE_INPUT_STREAM_FLAGS Flags)
    {
        ...
        IncludeResult* pNewInclude           = nullptr;
        const String   NormalizedIncludePath = NormalizeShaderSourcePath(IncludePath);
        if (!NormalizedIncludePath.empty())
        {
            RefCntAutoPtr<IFileStream> pSourceStream;
            m_pInputStreamFactory->CreateInputStream2(NormalizedIncludePath.c_str(), Flags, &pSourceStream);
            if (pSourceStream != nullptr)
            {
                RefCntAutoPtr<DataBlobImpl> pFileData = DataBlobImpl::Create();
                pSourceStream->ReadBlob(pFileData);
                pNewInclude = new IncludeResult{
