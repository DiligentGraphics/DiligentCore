...
{
public:
    const char* name() const override { return "strip-reflect-2"; }
    ...
    Status Process() override
    {
        ...
        for (auto& inst : context()->module()->extensions())
        {
            ...
            else if (0 == std::strcmp(ext_name, "SPV_GOOGLE_decorate_string"))
            {
                to_remove.push_back(&inst);
            }
...
std::vector<uint32_t> StripReflection(const std::vector<uint32_t>& OriginalSPIRV)
{
    ...
    SpirvOptimizer.RegisterPass(spvtools::MakeUnique<spvtools::Optimizer::PassToken::Impl>(spvtools::MakeUnique<StripReflectInfoPass2>()));
