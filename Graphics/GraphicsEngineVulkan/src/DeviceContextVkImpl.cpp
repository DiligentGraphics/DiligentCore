...
void DeviceContextVkImpl::TransitionImageLayout(ITexture* pTexture, VkImageLayout NewLayout)
{
    ...
    {
        TransitionTextureState(*pTextureVk, RESOURCE_STATE_UNKNOWN, NewState, RESOURCE_STATE_FLAG_UPDATE_STATE);
    }
}
                                                 ...
                                                 VkImageSubresourceRange* pSubresRange /* = nullptr*/)
{
    ...
    auto OldLayout = (Flags & RESOURCE_STATE_FLAG_DISCARD_CONTENT) != 0 ? VK_IMAGE_LAYOUT_UNDEFINED : ResourceStateToVkImageLayout(OldState);
    ...
    if (((OldState & NewState) != NewState) || OldLayout != NewLayout || AfterWrite)
    {
        ...
        if ((Flags & RESOURCE_STATE_FLAG_UPDATE_STATE) != 0)
        {
                                                         ...
                                                         const char*                    OperationName)
{
    if (TransitionMode == RESOURCE_STATE_TRANSITION_MODE_TRANSITION)
    {
        ...
        {
            TransitionTextureState(Texture, RESOURCE_STATE_UNKNOWN, RequiredState, RESOURCE_STATE_FLAG_UPDATE_STATE);
...
void DeviceContextVkImpl::TransitionResourceStates(Uint32 BarrierCount, const StateTransitionDesc* pResourceBarriers)
{
    ...
    for (Uint32 i = 0; i < BarrierCount; ++i)
    {
        ...
        if (Barrier.TransitionType == STATE_TRANSITION_TYPE_BEGIN)
        {
            ...
            VERIFY((Barrier.Flags & RESOURCE_STATE_FLAG_UPDATE_STATE) == 0, "Resource state can't be updated in begin-split barrier");
            continue;
        }
        ...
        {
            TransitionBufferState(*pBuffer, Barrier.OldState, Barrier.NewState, (Barrier.Flags & RESOURCE_STATE_FLAG_UPDATE_STATE) != 0);
        }
        else if (RefCntAutoPtr<BottomLevelASVkImpl> pBottomLevelAS{Barrier.pResource, IID_BottomLevelAS})
        {
            TransitionBLASState(*pBottomLevelAS, Barrier.OldState, Barrier.NewState, (Barrier.Flags & RESOURCE_STATE_FLAG_UPDATE_STATE) != 0);
        }
        else if (RefCntAutoPtr<TopLevelASVkImpl> pTopLevelAS{Barrier.pResource, IID_TopLevelAS})
        {
            TransitionTLASState(*pTopLevelAS, Barrier.OldState, Barrier.NewState, (Barrier.Flags & RESOURCE_STATE_FLAG_UPDATE_STATE) != 0);
        }
        else
        {
