...
if [ "$1" = "win" ]; then
  if [ "$CMAKE_GENERATOR" == "" ]; then
    cmake $GITHUB_WORKSPACE -G "Visual Studio 17 2022" -A $3 -DCMAKE_BUILD_TYPE=$4 $5 -DDILIGENT_NO_FORMAT_VALIDATION=ON -DCMAKE_INSTALL_PREFIX=install
  else
...
if [ "$1" = "uwp" ]; then
  cmake $GITHUB_WORKSPACE -G "Visual Studio 17 2022" -A $3 -DCMAKE_BUILD_TYPE=$4 $5 -DDILIGENT_NO_FORMAT_VALIDATION=ON -DCMAKE_INSTALL_PREFIX=install -DCMAKE_SYSTEM_NAME=WindowsStore -DCMAKE_SYSTEM_VERSION=10.0
fi
