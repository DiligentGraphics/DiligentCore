...
{
    TestingEnvironmentMtl* const pEnv      = TestingEnvironmentMtl::GetInstance();
    id<MTLDevice> const          mtlDevice = pEnv->GetMtlDevice();

    if (@available(macos 13.0, ios 16.0, *))
    {
        @autoreleasepool
        {
            NSString* progSrc = [NSString stringWithUTF8String:MSL::MeshShaderTest.c_str()];
            NSError* errors = nil;
            ...
            id<MTLFunction> msFunc = [library newFunctionWithName:@"MSmain"];
            ASSERT_TRUE(msFunc != nil);
            [msFunc autorelease];
            id<MTLFunction> psFunc = [library newFunctionWithName:@"PSmain"];
            ASSERT_TRUE(psFunc != nil);
            [psFunc autorelease];

            TestingSwapChainMtl* pTestingSwapChainMtl = ClassPtrCast<TestingSwapChainMtl>(pSwapChain);
            const SwapChainDesc& SCDesc               = pTestingSwapChainMtl->GetDesc();
            ITextureView*        pRTV                 = pTestingSwapChainMtl->GetCurrentBackBufferRTV();
            id<MTLTexture>       mtlBackBuffer        = ClassPtrCast<ITextureViewMtl>(pRTV)->GetMtlTexture();

            MTLMeshRenderPipelineDescriptor* meshPipeDesc = [[MTLMeshRenderPipelineDescriptor alloc] init];
            ...
            id <MTLRenderPipelineState> meshPipeline =
                [mtlDevice newRenderPipelineStateWithMeshDescriptor:meshPipeDesc
            ...
            id<MTLCommandQueue>         mtlCommandQueue  = pEnv->GetMtlCommandQueue();
            id<MTLCommandBuffer>        mtlCommandBuffer = [mtlCommandQueue commandBuffer];
            id<MTLRenderCommandEncoder> renderEncoder    = [mtlCommandBuffer renderCommandEncoderWithDescriptor:renderPassDesc];
...
void AmplificationShaderDrawReferenceMtl(ISwapChain* pSwapChain)
{
    ...
    if (@available(macos 13.0, ios 16.0, *))
    {
        @autoreleasepool
        {
            NSString* progSrc = [NSString stringWithUTF8String:MSL::AmplificationShaderTest.c_str()];
            NSError* errors = nil;
            MTLCompileOptions* options = [[MTLCompileOptions alloc] init];
            options.languageVersion = MTLLanguageVersion3_0;
            id<MTLLibrary> library = [mtlDevice newLibraryWithSource:progSrc
            ...
            id<MTLFunction> objFunc = [library newFunctionWithName:@"OBJmain"];
            ASSERT_TRUE(objFunc != nil);
            [objFunc autorelease];
            id<MTLFunction> msFunc = [library newFunctionWithName:@"AmpMSmain"];
            ASSERT_TRUE(msFunc != nil);
            [msFunc autorelease];
            id<MTLFunction> psFunc = [library newFunctionWithName:@"AmpPSmain"];
            ASSERT_TRUE(psFunc != nil);
            [psFunc autorelease];
