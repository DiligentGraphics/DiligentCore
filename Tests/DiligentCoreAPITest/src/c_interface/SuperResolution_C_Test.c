...
int TestSuperResolutionCInterface(struct ISuperResolution* pUpscaler)
{
    ...
    int num_errors =
        ...
        TestDeviceObjectCInterface((struct IDeviceObject*)pUpscaler);
    ...
    Desc = *IDeviceObject_GetDesc(pUpscaler);
    if (Desc.Name == NULL)
        ++num_errors;

    UniqueId = IDeviceObject_GetUniqueID(pUpscaler);
    if (UniqueId == 0)
        ++num_errors;
    ...
    ISuperResolution_GetOptimalJitterPattern(pUpscaler, 0, &JitterX, &JitterY);
    (void)JitterX;
    (void)JitterY;
